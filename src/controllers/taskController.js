...
//       priority: priority || 'Medium',
//       status: status || 'Pending',
//       assignee: req.userId, // Logged-in user is the assignee (creator)
//       assignedTo: assignedTo || null,
//     });
...
//     if (assignedToUser && assignedTo !== task.assignedTo?.toString()) {
//       const io = req.app.get('io');
//       io.to(assignedTo).emit('notification', {
//         message: `You have been assigned to complete task: ${task.title}`,
//         taskId: task._id,
...
exports.createTask = async (req, res) => {
  ...
  try {
    ...
    await task.save();
    console.log('Task created:', { taskId: task._id, assignedTo }); // Debug log

    if (assignedToUser) {
      const io = req.app.get('io');
      console.log('Emitting notification to:', assignedTo); // Debug log
      io.to(assignedTo).emit('notification', {
        message: `You have been assigned to complete task: ${title}`,
        taskId: task._id,
        type: 'task',
...
exports.getTasks = async (req, res) => {
  ...
  try {
    ...
    const tasks = await Task.find(query)
      .populate('assignee', 'username')
      .populate('assignedTo', 'username');
    console.log('Fetched tasks:', tasks.map((t) => ({ id: t._id, assignedTo: t.assignedTo }))); // Debug log
    res.json(tasks);
  } catch (error) {
    console.error('Get tasks error:', error);
    res.status(500).json({ message: 'Server error' });
...
exports.updateTask = async (req, res) => {
  ...
  try {
    ...
    await task.save();
    console.log('Task updated:', { taskId: task._id, assignedTo: task.assignedTo }); // Debug log
...
exports.deleteTask = async (req, res) => {
  ...
  try {
    ...
    await task.remove();
    console.log('Task deleted:', id); // Debug log
    res.json({ message: 'Task deleted successfully' });
  } catch (error) {
    console.error('Delete task error:', error);
    res.status(500).json({ message: 'Server error during task deletion', error: error.message });
